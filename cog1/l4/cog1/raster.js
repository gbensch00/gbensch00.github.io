...
function(exports, shader, framebuffer, data) {
	...
	function assembleIntersectionForScanline(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
		...
		for(var i = polygon.length - 1; i > 0; i--) {
			...
			var preCalcDerivate = calcDerivative(Math.floor(startPoint[1]), Math.floor(endPoint[1]));
			if (preCalcDerivate != 0) {
				derivative = preCalcDerivate;
				break;
			}
		}
		...
		for(var i = 0; i < polygon.length; i++) {
			...
			startPoint = vertices[polygon[i]]
			// Connect edge to next or to first vertex to close the polygon.
			var nextVertexIndex = (i < polygon.length - 1) ? i + 1 : 0;
			...
			drawLineBresenham(currX, currY, currZ, nextX, nextY, nextZ, color, true, edgeStartVertexIndex, edgeEndVertexIndex, undefined, undefined);

			// Calculate current and save last derivative.
			//console.log(i + ": derivative:" + derivative + " lastDerivative " + lastDerivative);
			...
			// Add end point of non horizontal edges.
				...
				//console.log(i + ": Add end point:" + nextX + ", " + nextY);
				addIntersection(nextX, nextY, nextZ, 1, edgeStartVertexIndex, edgeEndVertexIndex);
			}
			...
			if (lastDerivative + derivative == 0) {
				addIntersection(currX, currY, nextZ, 0, edgeStartVertexIndex, edgeEndVertexIndex);
			}
